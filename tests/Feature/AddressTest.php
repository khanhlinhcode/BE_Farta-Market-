...
test('customer address list and default endpoint never expose or alter another owner', function () {
    $owner = User::factory()->customer()->create();
    $other = User::factory()->customer()->create();
    $foreign = $owner->addresses()->create(addressPayload(['is_default' => true]));
    $mine = $other->addresses()->create(addressPayload(['label' => 'Mine', 'is_default' => true]));
    Sanctum::actingAs($other);

    $this->getJson('/api/addresses')->assertOk()->assertJsonCount(1, 'data')->assertJsonPath('data.0.id', $mine->id);
    $this->patchJson("/api/addresses/{$foreign->id}/set-default")->assertNotFound();
    expect($foreign->fresh()->is_default)->toBeTrue()->and($mine->fresh()->is_default)->toBeTrue();
});
