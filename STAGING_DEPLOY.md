...
6. Cloudflare Turnstile is enabled for guest checkout on farta-storefront.pages.dev. Its secret is stored outside Git and injected only into the API; the public site key is included only at storefront build time. A fake token returns 422 without changing inventory. Complete one successful interactive guest checkout in a normal browser because headless challenge completion is intentionally unreliable. Configure outbound mail and test email verification plus queued mail. Use VNPay sandbox credentials and a signed callback before enabling online payment. Supply an AI provider or reachable Ollama endpoint before enabling chat; /api/chat/health currently returns 503.
7. Use the completed post-sync backup for an isolated restore drill before accepting real orders. Compare migration status and core table counts after restore. Keep database private and monitor free-plan usage and quotas.

Until mail, an interactive Turnstile checkout, VNPay sandbox, AI, credential rotation, an isolated restore drill, and end-to-end payment/order checks are complete, this is a staging/demo environment, not production-ready.
