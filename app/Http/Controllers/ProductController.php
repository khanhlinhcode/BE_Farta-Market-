...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        } elseif (array_key_exists('active', $filters)) {
    ...
    public function uploadImage(Request $request, ProductModel $product)
    {
        ...
        try {
            DB::transaction(function () use ($product, $uploaded) {
                $product->images()->update(['is_primary' => false]);
                $this->createProductImage($product, $uploaded, true);
                $product->update(['img' => $uploaded['url']]);
            });
        } catch (Throwable $exception) {
            $this->cleanupCloudinaryUploads([$uploaded]);
    ...
    public function uploadImages(Request $request, ProductModel $product)
    {
        ...
        try {
            foreach ($data['images'] as $image) {
                $uploads[] = $this->cloudinary->upload($image, $product->id);
            }

            $createdImages = DB::transaction(function () use ($product, $uploads, $shouldSetPrimary) {
                $created = [];

                foreach ($uploads as $index => $uploaded) {
                    $isPrimary = $shouldSetPrimary && $index === 0;
                    $created[] = $this->createProductImage($product, $uploaded, $isPrimary);
    ...
    public function setPrimaryImage(ProductImage $image)
    {
        ...
        DB::transaction(function () use ($product, $image) {
            ...
            $product->update(['img' => $image->url]);
        });

        return response()->json(['product' => $product->fresh(['category', 'images'])]);
    }

    public function reorderImages(Request $request, ProductModel $product)
    {
        $data = $request->validate([
            'image_ids' => ['required', 'array'],
            'image_ids.*' => ['required', 'integer', 'distinct'],
        ]);
        $storedIds = $product->images()->pluck('id')->map(fn ($id) => (int) $id)->sort()->values();
        $requestedIds = collect($data['image_ids'])->map(fn ($id) => (int) $id)->sort()->values();

        if ($storedIds->all() !== $requestedIds->all()) {
            throw ValidationException::withMessages(['image_ids' => 'Danh sách ảnh không thuộc đầy đủ sản phẩm này.']);
        }

        DB::transaction(function () use ($product, $data) {
            foreach ($data['image_ids'] as $order => $imageId) {
                $product->images()->whereKey($imageId)->update(['sort_order' => $order]);
            }
        });
