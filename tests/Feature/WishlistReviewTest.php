...
test('inactive products cannot be added and disappear from wishlist results', function () {
    $user = User::factory()->customer()->create();
    $product = createCommerceProduct(['is_active' => false]);
    Sanctum::actingAs($user);

    $this->postJson("/api/wishlist/{$product->id}")->assertNotFound();
    Wishlist::create(['user_id' => $user->id, 'product_id' => $product->id]);
    $this->getJson('/api/wishlist')->assertOk()
        ->assertJsonPath('data', [])->assertJsonPath('ids', []);
});
